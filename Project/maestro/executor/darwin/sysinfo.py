"""macOS system-metric backend. Volume via AppleScript (no extra dependency)."""
...
def read_metric(metric: str, path: str = "~") -> dict:
    ...
    if metric == "os":
        return {"system": platform.system(), "release": platform.mac_ver()[0] or
                platform.release(), "machine": platform.machine()}
    if metric == "cpu":
        try:
            import psutil
